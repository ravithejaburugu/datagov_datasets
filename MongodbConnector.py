...
class mongodbConnector():
    ...
    def __init__(self):
        ...
        try:
            ...
            self.client = MongoClient(mongo_uri)  # , ssl=ssl_required,
                                 #replicaSet='Cluster0-shard-0')

            # Authenticate MongoDB (Optional)
            if requires_auth == 'true':
                self.client.the_database.authenticate(mongo_username,
                                                 mongo_password,
                                                 source=mongo_auth_source,
                                                 mechanism=mongo_auth_mech
                                                 )
            self.mongo_inst = self.client[db_name]
        except IOError:
            raise
    ...
    def bulk_mongo_update(self, mongo_colln, obj_list):
        ...
        try:
            for new_obj in obj_list:
                mongo_colln.update_one({'_id': self.get_id(mongo_colln, new_obj)},
                                       {"$set": new_obj},
                                       upsert=True)
        except:
            raise
    ...
    def get_id(self, mongo_colln, obj):
        """While updating existing Object, needs its object Id."""
        obj_cur = mongo_colln.find({"title": obj["title"]})
        m_obj = obj_cur.next()
        return m_obj['_id']

    def get_collections_documents_count(self):
        """To fetch all the list of collections in the given database,
        with the number of documents/records in each collection."""
        mongo = mongodbConnector()
        coln_docs_count = {}
        try:
            for colln_name in self.mongo_inst.collection_names():
                data_colln = mongo.initialize_mongo(colln_name)
                coln_docs_count[colln_name] = data_colln.count()
        except:
            raise
...
if __name__ == "__main__":
    mongodbConnector().get_collections_documents_count()